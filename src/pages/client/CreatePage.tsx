...
/* ================================================================
   AI Full Flow Modal — Topic & Tone → Caption → Visuals → Hashtags
   ================================================================ */
...
type AiPayload = { caption: string; hashtags: string[]; media: string | null };

function AiFlowModal({
  open, onOpenChange, selectedPlatforms, onApply,
}: {
  open: boolean;
  onOpenChange: (o: boolean) => void;
  selectedPlatforms: PlatformKey[];
  onApply: (p: AiPayload) => void;
}) {
  ...
  const reset = () => {
    setStep(0); setDescription(""); setTones([]); setStyle("Short and punchy");
    setAudience(""); setKeywords(""); setCaptionOptions([]); setChosenCaption("");
    setVisualOptions([]); setChosenVisual(null); setHashtagOptions([]); setChosenHashtags([]);
  };

  const STEPS = ["Topic & Tone", "Caption", "Visuals", "Hashtags"];
  ...
  useMemo(() => {
    if (open && step === 1 && captionOptions.length === 0 && !captionLoading) generateCaptions();
    if (open && step === 2 && visualOptions.length === 0 && !visualLoading) generateVisuals();
    if (open && step === 3 && hashtagOptions.length === 0) generateHashtags();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, step]);

  const finish = () => {
    onApply({ caption: chosenCaption, hashtags: chosenHashtags, media: chosenVisual });
    reset();
  };
        ...
        <div className="px-6 py-4 border-b border-border bg-gradient-to-r from-primary/5 to-transparent">
          <div className="flex items-center justify-between mb-3">
            <DialogTitle className="flex items-center gap-2 text-lg">
              <Sparkles className="w-5 h-5 text-primary" /> Create with AI
            </DialogTitle>
            ...
            <>
              <div>
                ...
                <textarea
                  ...
                  className="w-full min-h-[100px] resize-none px-3 py-2.5 rounded-lg border border-input text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                />
              ...
              <div className="rounded-lg border border-border p-4 bg-muted/20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={audience} onChange={(e) => setAudience(e.target.value)} placeholder="e.g., Young professionals" className="mt-1 w-full px-3 py-2 rounded-lg border border-input text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                  </div>
                  <div>
                    <label className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground">Specific keywords</label>
                    <input value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="e.g., AI, automation, growth" className="mt-1 w-full px-3 py-2 rounded-lg border border-input text-sm outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                  </div>
                </div>
              </div>
            </>
          ...
          {step === 3 && (
            <>
              ...
              <div className="rounded-lg border border-border p-3 bg-muted/20">
                <p className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground mb-2">Final preview</p>
                <p className="text-sm leading-relaxed whitespace-pre-wrap">
                  {chosenCaption}
                  {chosenHashtags.length > 0 && `\n\n${chosenHashtags.join(" ")}`}
                </p>
                {chosenVisual && <img src={chosenVisual} alt="Selected" className="mt-3 w-32 rounded-md border border-border" />}
              </div>
            </>
          )}
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {step < 3 ? (
              ...
              <button
                ...
                className="px-5 py-2 rounded-lg gradient-coral text-primary-foreground text-sm font-semibold shadow-coral hover:opacity-90 active:scale-[0.98] transition-all flex items-center gap-1.5"
              >
                <Check className="w-4 h-4" /> Apply to composer
              </button>
            )}
          </div>
        </div>
