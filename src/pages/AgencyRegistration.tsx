    ...
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="w-[520px] bg-card border border-border rounded-2xl p-12 shadow-sm">
        <div className="flex flex-col items-center mb-6">
          ...
          <span className="mt-2 text-xs text-text-secondary bg-muted rounded-full px-3 py-1">invited@agency.com</span>
        </div>

        <h1 className="text-2xl font-semibold text-foreground mb-6">Set up your agency account</h1>

        <div className="space-y-4">
          <div><label className="text-sm text-text-secondary mb-1.5 block">Agency Name</label><input className="input-dark" defaultValue="Digital Spark Agency" /></div>
          <div><label className="text-sm text-text-secondary mb-1.5 block">Your Full Name</label><input className="input-dark" placeholder="Enter your full name" /></div>
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted"><Eye className="h-4 w-4" /></button>
            </div>
            {password && (
              <div className="mt-2">
                <div className="flex gap-1">{[1, 2, 3].map(i => <div key={i} className={`h-1 flex-1 rounded-full ${i <= strength ? strengthColors[strength] : 'bg-border'}`} />)}</div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted"><Eye className="h-4 w-4" /></button>
            </div>
          </div>
          ...
          <Button className="w-full mt-2" disabled={!agreed}>Create Agency Account</Button>
        </div>
      </div>
    </div>
  );
